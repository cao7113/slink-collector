document.addEventListener("DOMContentLoaded", () => {
  ...
  (
    document.getElementById("optionsLink") as HTMLAnchorElement
  ...
  chrome.storage.sync.get("configs", (data) => {
    ...
    if (!activeConfig) {
      // alert(`No active configuration found, should go to options`);
      // const optionsUrl = `chrome-extension://${chrome.runtime.id}/options.html`;
      (document.getElementById("collect") as HTMLDivElement).innerHTML = `
      <div class="text-red-400">
        No active configuration found, should go to options page!
      </div>
    `;
      return;
    }
